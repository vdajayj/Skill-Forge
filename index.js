...
const key = "Ajay_vuppu"; // secret key
...
//connect to mongoDB
mongoose.connect(
  "mongodb+srv://ajayvuppu3211:dB482mlXJp82PgCe@cluster0.2xgusae.mongodb.net/",
...
app.post("/admin/signup", async (req, res) => {
  ...
  if (admin) res.status(403).json({ message: "Admin already exists" });
    ...
    await newAdmin.save();
    const token = jwt.sign({ username, role: "admin" }, key, {
      expiresIn: "1h",
    });
...
app.post("/admin/login", async (req, res) => {
  ...
  if (!admin) res.status(404).json({ message: "Invalid username or password" });
  else {
    const token = jwt.sign({ username, role: "admin" }, key, {
      expiresIn: "1h",
    });
...
app.post("/users/signup", async (req, res) => {
  ...
  if (user) res.status(403).json({ message: "User exists!!" });
  else {
